...
# 1. Remove FOOD_DATABASE and Suggestions
pattern_db = re.compile(
    r"// ============================================================\n// STRUCTURED FOOD DATABASE.*?(?=\n// ============================================================\n// STYLE TOKENS)",
    re.DOTALL
)
code = pattern_db.sub("", code)

# 2. Replace FoodScanner completely
new_scanner = """// ============================================================
// AI FOOD SCANNER
...
const FoodScanner = ({ onScanComplete, profile }: { onScanComplete: (meal: Meal) => void; profile: HealthProfile | null }) => {
  ...
  const scan = async () => {
    ...
    try {
      ...
      const apiRes = await fetch('http://127.0.0.1:8000/api/analyze-meal', {
        method: 'POST',
        body: formData
      });
...
pattern_scanner = re.compile(
    r"// ============================================================\n// SMART FOOD CLASSIFIER.*?// ============================================================\n// DASHBOARD",
    re.DOTALL
)
code = pattern_scanner.sub(new_scanner + "\n// ============================================================\n// DASHBOARD", code)

with open("src/App.tsx", "w", encoding="utf-8") as f:
    f.write(code)

print("App.tsx refactored successfully.", len(code))
