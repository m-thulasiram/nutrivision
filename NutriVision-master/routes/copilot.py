...
async def mock_copilot_stream(message: str, context: dict):
    """Fallback mock stream when OpenAI API key is not configured."""
    remaining_protein = context.get("remaining_protein", 80)
    state = context.get("preferred_state", "Tamil Nadu")
    
    if "what" in message.lower() and "eat" in message.lower():
        response = (
            f"Based on your profile, you need {remaining_protein}g more protein "
            f"today. For {state}, I recommend Dal Tadka with rice — high protein, "
            f"fits your vegetarian preference perfectly. About 320 kcal and 18g "
            f"protein per serving."
        )
    else:
        response = (
            f"I'm your NutriVision nutrition assistant. Ask me what to eat, "
            f"log a meal, or check your progress!"
        )
...
async def copilot_chat(
    ...
    if not api_key or api_key == "your_key_here":
        return StreamingResponse(
            mock_copilot_stream(request.message, user_context),
